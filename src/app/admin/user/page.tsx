...
//   const [formData, setFormData] = useState({ name: '', email: '', role: 'CUSTOMER' as 'ADMIN' | 'CUSTOMER' | 'RIDER' });
...
//         setUsers(users.map(u => u.publicId === publicId ? { ...u, ...formData } : u));
...
//         setUsers(users.filter(u => u.publicId !== publicId));
//       } else {
...
//                         Delete
//                       </button>
//                       {user.details?.pendingDetails && user.details.pendingDetails.length > 0 && user.details.pendingDetails.map((pending, index) => (
//                         <div key={index} className="mt-2 p-2 bg-yellow-100 border border-yellow-400 rounded">
//                           <p><strong>Pending Address #{index + 1}:</strong> {pending.address}</p>
